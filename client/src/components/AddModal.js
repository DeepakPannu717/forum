...
export default function AddModal({ type, onAdd, onClose, categories = [] }) {
  ...
  const validate = () => {
    ...
    if (type === "category" && modalMode === "subcategory" && !formData.categoryId) {
      newErrors.categoryId = "Parent category is required";
    }
    
    // Validate subcategory for topics when a category is selected
    if (type === "topic" && formData.categoryId) {
      const selectedCategory = categories.find(c => c._id === formData.categoryId);
      if (selectedCategory?.subcategories?.length > 0 && !formData.subcategoryId) {
  ...
  const handleSubmit = () => {
    ...
    if (type === "topic") {
      ...
      if (modalMode === "subcategory") {
        payload = {
          name: formData.name,
          parentId: formData.categoryId // Use parentId for subcategories
        };
      } else {
        payload = {
          name: formData.name
  ...
  return (
    <Modal show onHide={onClose} size="lg" scrollable>
          ...
          {type === 'category' ? (
            <div className="d-flex align-items-center gap-3">
              ...
              <Button
                ...
                size="sm"
                onClick={() => setModalMode(modalMode === 'category' ? 'subcategory' : 'category')}
              >
                {modalMode === 'category' ? 'Switch to Subcategory' : 'Switch to Category'}
              </Button>
            </div>
          ) : (
            `Add ${type}`
      ...
      <Modal.Body>
        <Form>
          {type === 'category' && modalMode === 'subcategory' ? (
            <>
            ...
            <Form.Group className="mb-3">
              <Form.Label>{type === 'category' ? 'Category' : type} Name</Form.Label>
              <Form.Control
                ref={inputRef}
                type="text"
                name="name"
